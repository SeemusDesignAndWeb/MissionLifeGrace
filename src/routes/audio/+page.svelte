...
<svelte:head>
	<title>{data.page?.title || 'Audio'} - Mission Life Grace</title>
				...
				{#each paginatedPodcasts as podcast}
					...
					>
						...
						<div class="flex items-center justify-between mt-2">
							{#if podcast.series}
								<span class="inline-block px-3 py-1 {isMostRecent ? 'bg-white text-brand-blue' : 'bg-primary/10 text-primary'} text-xs rounded-full">
									{podcast.series}
								</span>
							{:else}
								<span></span>
...
{:else}
	<section class="py-20 bg-gray-50">
		<div class="container mx-auto px-4 text-center">
			<p class="text-gray-600">No messages available yet. Check back soon!</p>
		</div>
	</section>
{/if}
